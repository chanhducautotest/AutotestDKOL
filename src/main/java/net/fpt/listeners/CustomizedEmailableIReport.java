...
import static net.fpt.utils.SendEmail.sendEmailByAPI;
...
public class CustomizedEmailableIReport implements IReporter{



    static final Logger logger = LoggerFactory.getLogger(CustomizedEmailableIReport.class);
    // ~ Instance fields ------------------------------------------------------
    ...
    private final Boolean enableEmailReport =  Boolean.getBoolean("enableEmailReport");
    private final String toEmail = System.getProperty("toEmailReport");
    private final String toCC = System.getProperty("toCCReport");
    private final String toBCC = System.getProperty("toBCCReport");
    private final String subjectEmailReport = System.getProperty("subjectEmailReport");

    // Singleton class for get/set data to report anywhere
    public String fullReportName;

    private static CustomizedEmailableIReport instance = null;

    private void CustomizedEmailableIReport(){


    }
    public static CustomizedEmailableIReport getInstance(){
        if(instance==null){
            instance = new CustomizedEmailableIReport();
        }
        return instance;
    }
    ...
    public void generateReport(List<XmlSuite> xml, List<ISuite> suites, String outdir) {
        try {
            outdir = System.getProperty("user.dir") + File.separator + "target" + File.separator + "reports" + File.separator + "emailable_report";
...
/*        ConfigUtil cr = ConfigUtil.getInstance();
        ...
        if(enableEmailReport){
            try {
                logger.info("enabled Email Report");
                String reportPath=System.getProperty("user.dir")+"/target/reports/emailable_report/emailable-report.html";
                String content = new String(Files.readAllBytes(Paths.get(reportPath)), StandardCharsets.UTF_8);
                sendEmailByAPI(toEmail,toCC,toBCC,subjectEmailReport,content);
            } catch (IOException e) {
                logger.error("Send Email Report: "+ e.getCause());
            }
        }
    }
        ...
        if (tests.getAllResults().size() > 0) {
            ...
            for (ITestNGMethod method : getMethodSet(tests, suite)) {
                ...
                buff.append("<td><a href=\"#m" + methodId + "\">" + qualifiedName(method) + " "
                        ...
                        + "</td><td class=\"numi\">" + resultSet.size() + "</td>"
                        + "<td>" + (count == 0 ? "" : count) + "</td>" + "<td>" + parameterString + "</td>" + "<td>"
                        + convertDate(start) + "</td>" + "<td class=\"numi\">" + (end - start) + "</td>" + "</tr>");
            }
            if (mq > 0) {
                cq += 1;
                m_out.print("<tr class=\"" + style + (cq % 2 == 0 ? "even" : "odd") + "\">" + "<td");
    ...
    public void generateSuiteSummaryReport(List<ISuite> suites) {
        ...
        for (ISuite suite : suites) {
            ...
            for (ISuiteResult r : tests.values()) {
                ...
                summaryCell(
                        formatter.format((overview.getEndDate().getTime() - overview.getStartDate().getTime()) / 1000.) + " s ~ "
                                + formatter.format(((overview.getEndDate().getTime() - overview.getStartDate().getTime()) / 1000.) / 60) + " mi",
                        true);
        ...
        if (qty_tests > 1) {
            ...
            summaryCell(formatter.format((time_end - time_start) / 1000.) + " s ~ "
                    + formatter.format(((time_end - time_start) / 1000.) / 60) + " mi",
                    true);
    ...
    protected void startHtml(PrintWriter out) {
        ...
        out.println("</head>");
        out.println("<body>");
        out.println("<p ><a href='https://hi-static.fpt.vn/sys/selenium-core/stag/"+ CustomizedEmailableIReport.getInstance().fullReportName+ "'> View Full Report</a></p>");
